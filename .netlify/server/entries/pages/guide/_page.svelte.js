...
const participant_guide_png = "/_app/immutable/assets/participant_guide.COAz8JVc.png";
const TITLE = "설문모아 가이드";
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `${$$result.head += `<!-- HEAD_svelte-3f9t3p_START -->${$$result.title = `<title>${escape(TITLE)}</title>`, ""}<meta name="description"${add_attribute("content", `${TITLE}`, 0)}><!-- HEAD_svelte-3f9t3p_END -->`, ""} ${validate_component(Page$1, "Header").$$render($$result, {}, {}, {
    center: () => {
      return `<h1 slot="center" class="font-semibold">${escape(TITLE)}</h1>`;
    },
    left: () => {
      return `<a href="/home" slot="left">${validate_component(Page$2, "Icon").$$render($$result, { name: "left_arrow" }, {}, {})}</a>`;
    }
  })} <div class="mt-5 flex justify-center"><button${add_attribute(
    "class",
