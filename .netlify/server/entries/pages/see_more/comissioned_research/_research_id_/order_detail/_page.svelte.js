...
import { f as format_date, c as comma, b as calculate_d_day } from "../../../../../../chunks/common.js";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `${validate_component(Page$1, "Header").$$render($$result, { nav_class: "border-0 bg-white" }, {}, {
      ...
      "class",
      `font-bold ${research__research_payment.status === "결제취소" ? "text-gray-900" : ""}`,
      0
    )}>${escape(research__research_payment.status)}</p> <button data-svelte-h="svelte-7he19p"><u class="text-sm text-gray-900">계좌복사</u></button></div> <p class="mt-0.5 text-sm text-gray-900" data-svelte-h="svelte-1xe2dyu">국민은행 10430104503406</p></div> <div class="mt-6 h-px bg-gray-300"></div> <div class="mx-6 py-6"><div class="flex items-center justify-between"> <p class="text-sm text-gray-800">${escape(format_date(research__research_payment.start_date))} ~ ${escape(format_date(research__research_payment.end_date))}</p></div> <p class="mt-3.5 line-clamp-2 font-semibold">${escape(research__research_payment.title)}</p> <div class="mt-4 flex">${research__research_payment.images.length > 0 ? `<img${add_attribute("src", research__research_payment.images[0].uri, 0)}${add_attribute("alt", research__research_payment.category, 0)} class="mr-5 h-20 w-20 flex-shrink-0 rounded-xl object-cover">` : `<img${add_attribute(
      "src",
      research__research_payment.category === "설문조사" ? research_category_png : etc_category_png,
      0
    )}${add_attribute("alt", research__research_payment.category, 0)} class="mr-5 h-20 w-20 flex-shrink-0 rounded-xl object-cover">`} <div class="flex text-sm"><div class="flex flex-col justify-between pr-4" data-svelte-h="svelte-2pfnns"><p class="font-semibold text-gray-800">보상금</p> <p class="font-semibold text-gray-800">소요 시간</p> <p class="font-semibold text-gray-800">마감 날짜</p> <p class="font-semibold text-gray-800">모집 정보</p></div> <div class="flex flex-col justify-between"><p class="text-base font-bold text-primary">${escape(comma(research__research_payment.price))}원</p> <p>${escape(research__research_payment.expected_time)}분</p> <p>D - ${escape(calculate_d_day(research__research_payment.end_date))}</p> <p>${escape(research__research_payment.min_age)} ~ ${escape(research__research_payment.max_age)}세 성별
					${escape(research__research_payment.gender)} ${escape(research__research_payment.recruitment_num)}명</p></div></div></div></div> <div class="h-2 bg-gray-300"></div> <div class="mx-5 py-6"><p class="mb-4 font-semibold" data-svelte-h="svelte-gfy7wi">입금자 정보</p> ${validate_component(Page$3, "Account").$$render(
      $$result,
      {
        supabase,
        session,
