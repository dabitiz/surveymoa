...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let { data } = $$props;
  let { rating_user, research } = data;
  let rating = rating_user.rating;
  let comment = rating_user.comment;
  ...
  return `${$$result.head += `<!-- HEAD_svelte-1lyn9uj_START -->${$$result.title = `<title>${escape(TITLE)} | 설문모아</title>`, ""}<meta name="description"${add_attribute("content", TITLE, 0)}><!-- HEAD_svelte-1lyn9uj_END -->`, ""} ${validate_component(Page$1, "Header").$$render($$result, {}, {}, {
    ...
    0
  )}${add_attribute("alt", research.category, 0)} class="h-[53px] w-[53px] flex-shrink-0 rounded-lg object-cover">`}</div> <div class="flex flex-col justify-between"><p class="font-semibold" data-svelte-h="svelte-4irxev">한국, 일본에 대한 이미지</p> <p><span class="font-semiborder-l-red-950 mr-2 text-sm">${escape($page.state.username)}</span><span class="text-sm text-gray-900">${escape(format_date($page.state.participant_date))}</span></p></div></div> <div class="mt-10 flex flex-col items-center"><p class="text-lg font-bold">${escape(rating === 1 ? "1.0 나쁨" : rating === 2 ? "2.0 별로" : rating === 3 ? "3.0 보통" : rating === 4 ? "4.0 좋음" : rating === 5 ? "5.0 최고" : null)}</p> <div class="rating mt-2 flex gap-2"><input ${rating === 1 ? "checked" : ""} type="radio" name="rating" class="mask mask-star-2 h-7 w-7 bg-secondary"> <input ${rating === 2 ? "checked" : ""} type="radio" name="rating" class="mask mask-star-2 h-7 w-7 bg-secondary"> <input ${rating === 3 ? "checked" : ""} type="radio" name="rating" class="mask mask-star-2 h-7 w-7 bg-secondary"> <input ${rating === 4 ? "checked" : ""} type="radio" name="rating" class="mask mask-star-2 h-7 w-7 bg-secondary"> <input ${rating === 5 ? "checked" : ""} type="radio" name="rating" class="mask mask-star-2 h-7 w-7 bg-secondary"></div></div> <div class="mt-6 px-4"><textarea class="textarea mt-2 h-[164px] w-full break-words bg-gray-50 px-6 py-5 text-sm focus:border-primary focus:outline-none">${escape(comment || "")}</textarea></div> <div class="fixed bottom-0 mx-auto w-full bg-white px-5 py-3.5 md:w-1/2"><div class="pb-safe"><button class="btn btn-primary w-full" data-svelte-h="svelte-1xahrdl">리뷰 수정</button></div></div></main>`;
});
export {
  Page as default
};
