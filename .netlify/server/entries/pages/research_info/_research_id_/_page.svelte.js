...
import {
	r as research_category_png,
	e as etc_category_png
} from "../../../../chunks/etc_category.js";
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	do {
		...
		$$rendered = `${(($$result.head += `<!-- HEAD_svelte-1lyn9uj_START -->${(($$result.title = `<title>${escape(TITLE)} | 설문모아</title>`), "")}<meta name="description"${add_attribute("content", TITLE, 0)}><!-- HEAD_svelte-1lyn9uj_END -->`), "")} ${validate_component(
			Page$1,
			"Header"
		).$$render(
			$$result,
...
스크리닝 질문에 응답해 주세요!`
				...
				{
					default: () => {
						return `<div class="px-5">${
							screening_info.selected_questions.length > 0
								? `<h3 class="mt-10 font-bold">${escape(screening_info.selected_questions[screening_info.page_count].question)}</h3> <div class="mt-4">${each(
										screening_info.selected_questions[screening_info.page_count].choices,
										(choice) => {
											return `<div class="py-2"><label><div class="${[
												...
												(!choice.status ? "bg-gray-50" : "") +
													" " +
													(choice.status ? "bg-[#e9f0ff]" : "") +
													" " +
													(choice.status ? "border" : "") +
													" " +
													(choice.status ? "border-primary" : "")
											]
												.join(" ")
												.trim()}"><input type="checkbox" class="mr-3 h-5 w-5 appearance-none rounded-full border-2 border-gray-300 bg-white checked:border-[6px] checked:border-primary"${add_attribute("checked", choice.status, 1)}> <p class="text-sm">${escape(choice.choice)}</p> </div></label> </div>`;
										}
									)}</div>`
								: ``
						}</div> <div class="sticky bottom-0 w-full bg-white px-5 py-3.5"><button class="btn btn-primary w-full" data-svelte-h="svelte-fgxeyx">다음</button></div>`;
					}
				}
			)}`;
	} while (!$$settled);
