<script>
	...
	const update_rating_user = async () => {
		const { error } = await supabase
			.from("rating_user")
			.update({ rating, comment })
			.eq("id", Number($page.params.rating_user_id));

		if (error) throw new Error(`Failed to update_rating_user: ${error.message}`);
	};
</script>
